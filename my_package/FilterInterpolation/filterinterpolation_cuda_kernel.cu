...
__global__ void FilterInterpolationLayer_gpu_forward_kernelfunc(
		...
		const scalar_t* __restrict__    input1,    		const scalar_t* __restrict__    input2,    	const scalar_t* __restrict__    input3, 	scalar_t*   output, scalar_t*   mask
		...
		if(x2 >= 0.0f && y2 >=0.0f && x2 <= (float)(w -1) && y2 <= (float)(h-1)
			...
			for (int c_i = 0 ; c_i < channel ; c_i++){
                ...
                output[off + c_i * input1_c_stride + h_i * input1_h_stride + w_i ] =
							...
							(1-alpha)*beta*BL +
							alpha*beta*BR;
                mask[off + c_i * input1_c_stride + h_i * input1_h_stride + w_i ] = 1;
...
//				}
			...
			for(int c_i = 0 ;  c_i < channel; c_i ++){
				output[off + c_i * input1_c_stride + h_i * input1_h_stride + w_i] = input1[off + c_i* input1_c_stride+ h_i * input1_h_stride + w_i];
		                mask[off + c_i * input1_c_stride + h_i * input1_h_stride + w_i] = 0;
                      }
		}
	}
	return ;
...
int FilterInterpolationLayer_gpu_forward_kernel(
		...
		at::Tensor&  input1,    		at::Tensor&  input2,    	at::Tensor&  input3, 	at::Tensor&  output, at::Tensor&  mask
...
FilterInterpolationLayer_gpu_forward_kernelfunc<<<grid,block,0, stream >>>(
			...
			input1.data<scalar_t>(),input2.data<scalar_t>(),input3.data<scalar_t>(), output.data<scalar_t>(), mask.data<scalar_t>()
			);
 					}));
