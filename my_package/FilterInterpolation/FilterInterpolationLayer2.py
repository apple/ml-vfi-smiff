...
class FilterInterpolationLayer2(Function):
    def __init__(self):
        super(FilterInterpolationLayer2,self).__init__()
    @staticmethod
    def forward(ctx, input1,input2,input3):
        ...
        if input1.is_cuda :
            ...
            mask = torch.cuda.FloatTensor().resize_(input1.size()).zero_()
            my_lib.FilterInterpolationLayer_gpu_forward(input1, input2, input3, output, mask)
        else:
            output = torch.FloatTensor(input1.data.size())
            mask = torch.FloatTensor().resize_(input1.size()).zero_()
            my_lib.FilterInterpolationLayer_cpu_forward(input1, input2, input3, output, mask)
    ...
    @staticmethod
    def backward(ctx, gradoutput, gradmask):
